...
import { Container, Paper, Typography, Button, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Grid, Box, Divider, Card, CardContent, Chip, IconButton, Tooltip, Alert, LinearProgress, Avatar, CircularProgress, Select, MenuItem, FormControl, InputLabel, Menu } from '@mui/material';
import { Calculate, Save, Refresh, Assessment, TrendingUp, AccountBalance, Payment, CurrencyRupee, Analytics, Today, Receipt, Dashboard, Search, MoreVert, FilterList, GetApp, PictureAsPdf, TableChart, BarChart, Edit, Delete, Person, Logout } from '@mui/icons-material';
...
const DaySales = () => {
  ...
  if (currentView === 'PasswordManager.js') {
    return <PasswordManager onNavigate={setCurrentView} />;
  }

  if (currentView === 'Reports.js') {
    ...
    <Box id="main-container" sx={{ display: 'flex', height: '100vh', bgcolor: '#FAFAFA', overflow: 'hidden' }}>
      ...
      <Box id="main-content" sx={{ flex: 1, p: 1 }}>
        ...
        <Box id="top-bar" sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          ...
          >
            <MenuItem onClick={handleLogout} sx={{ fontSize: '0.8rem', gap: 1 }}>
